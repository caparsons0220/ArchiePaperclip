...
export async function runDatabaseBackup(opts: RunDatabaseBackupOptions): Promise<RunDatabaseBackupResult> {
  ...
  let sql = createPostgresClient(opts.connectionString, { max: 1, connect_timeout: connectTimeout });
  let sqlClosed = false;
  ...
  try {
    if (backupEngine === "pg_dump" || (backupEngine === "auto" && canUsePgDump)) {
      ...
      try {
        ...
        sql = createPostgresClient(opts.connectionString, { max: 1, connect_timeout: connectTimeout });
        sqlClosed = false;
      }
    }
    ...
    for (const { schema_name, tablename } of tables) {
      ...
      if (backupEngine !== "javascript" && nullifiedColumns.size === 0) {
        ...
        const copySql = createPostgresClient(opts.connectionString, { max: 1, connect_timeout: connectTimeout });
        try {
...
export async function runDatabaseRestore(opts: RunDatabaseRestoreOptions): Promise<void> {
  ...
  const sql = createPostgresClient(opts.connectionString, { max: 1, connect_timeout: connectTimeout });
